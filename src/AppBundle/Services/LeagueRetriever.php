<?php

namespace AppBundle\Services;

use Doctrine\ORM\EntityManager;
use AppBundle\Entity\League;
use AppBundle\Entity\FootballTeam;

class LeagueRetriever
{
    /**
     * @var EntityManager
     */
    private $entityManager;


    public function __construct(
        EntityManager $entityManager
    ) {
        $this->entityManager = $entityManager;
    }


    public function getFootballTeamList($leagueId){

        /** @var League|null $league */
        $league = $this->entityManager->getRepository(League::class)
            ->find($leagueId);

        if($league){
            $footballTeams = $league->getFootballTeams();
            $resultArray = [];
            /** @var FootballTeam $footballTeam */
            foreach ($footballTeams as $footballTeam){
                $record = [];
                $record['name'] = $footballTeam->getName();
                $record['strip'] =$footballTeam->getStrip();
                $resultArray[] = $record;
            }
            return $resultArray;
        }else{
            throw new \Exception('League not found!');
        }
    }
}
